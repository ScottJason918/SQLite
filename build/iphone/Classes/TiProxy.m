...
//Rare exceptions to indicate a bug in the _sqlite code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_sqlite = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_sqlite]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.sqlite.module"
